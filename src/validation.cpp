...
#if defined(NDEBUG)
# error "Slimcoin cannot be compiled without assertions."
#endif
...
bool MemPoolAccept::PolicyScriptChecks(ATMPArgs& args, Workspace& ws, PrecomputedTransactionData& txdata)
{
    ...
    // slimcoin: if transaction is after version 0.8 fork, verify SCRIPT_VERIFY_LOW_S
    // slmTODO move back to policy.h after 0.8 is active
...
{
    if (fUpdateUI)
        uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED); // slimcoin: we are using arguments that will have no effects in updateAlert()
#if HAVE_SYSTEM
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // slimcoin
            undo.nTime = alternate.nTime;           // slimcoin
        } else {
...
bool SlimcoinContextualBlockChecks(const CBlock& block, BlockValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->nStakeModifier == 0 && pindex->nStakeModifierChecksum == 0 && !SlimcoinContextualBlockChecks(block, state, pindex, fJustCheck))
    ...
    // slimcoin: coinbase reward check relocated to CheckBlock()
...
bool CheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // slimcoin: first coinbase output should be empty if proof-of-stake block
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    //slmTODO - move this somewhere in the upper calls, where pfrom is visible
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    // slimcoin: we should only accept blocks that can be connected to a prev block with validated PoS
    ...
    // slimcoin: check PoS
    if (fCheckPoS && !SlimcoinContextualBlockChecks(block, state, pindex, false)) {
        pindex->nStatus |= BLOCK_FAILED_VALID;
        setDirtyBlockIndex.insert(pindex);
