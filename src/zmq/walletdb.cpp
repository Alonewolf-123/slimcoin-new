...
//POB Code Start
bool WalletBatch::ReadBurnTx(uint256 hash, CWalletTx& wtx)
{
    return m_batch.Read(std::make_pair(std::string("burnTx"), hash), wtx);
}
bool WalletBatch::WriteBurnTx(uint256 hash, const CWalletTx& wtx)
{
    return WriteIC(std::make_pair(std::string("burnTx"), hash), wtx);
}
bool WalletBatch::EraseBurnTx(uint256 hash)
{
    return EraseIC(std::make_pair(std::string("burnTx"), hash));
}
//POB Code End
