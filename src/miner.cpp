...
// slimcoin: if pwallet != NULL it will attempt to create coinstake
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool* pfPoSCancel)
{
    ...
    if (pwallet)  // attemp to find a coinstake
    {
        *pfPoSCancel = true;
        ...
        if (*pfPoSCancel)
            return nullptr; // slimcoin: there is no point to continue if we failed to create coinstake
...
void PoSMiner(std::shared_ptr<CWallet> pwallet, CConnman* connman, CTxMemPool* mempool)
{
    ...
    util::ThreadRename("slimcoin-stake-minter");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                ...
                uiInterface.NotifyAlertChanged(uint256(), CT_UPDATED);
                LogPrintf("Error in SlimcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                if (!connman->interruptNet.sleep_for(std::chrono::seconds(10)))
                   return;
    ...
    {
        LogPrintf("SlimcoinMiner terminated\n");
    return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SlimcoinMiner runtime error: %s\n", e.what());
        return;
    }
}
