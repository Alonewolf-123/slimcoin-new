 ...
 */
static const char* BITCOIN_PID_FILENAME = "slimcoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/slimcoin/slimcoin>";
...
/** Sanity checks
 *  Ensure that Slimcoin is running in a usable environment with all
...
{
    // Make sure only a single Slimcoin process is using the data directory.
...
bool AppInitSanityChecks()
{
    ...
    // slimcoin: init hash seed
    slimcoinRandseed = GetRand(1 << 30);

#ifdef ENABLE_CHECKPOINTS
    // slimcoin: moved here because ECC need to be initialized to execute this
    if (gArgs.IsArgSet("-checkpointkey")) // slimcoin: checkpoint master priv key
    {
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if slimcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if slimcoin is started while in a temporary directory.\n",
