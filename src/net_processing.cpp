...
namespace {
    ...
    /** slimcoin: blocks that are waiting to be processed, the key points to previous CBlockIndex entry */
    struct WaitElement {
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // slimcoin: set/unset network serialization mode for new clients
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            // slimcoin: quick check to see if we should ban peers for PoS spam
    ...
    if (msg_type == NetMsgType::BLOCK)
    {
...

        {
            ...
            // slimcoin: store in memory until we can connect it to some chain
        ...
        // slimcoin: accept as many blocks as we possibly can from mapBlocksWait
        while (fContinue) {
            fContinue = false;
            ...
            {
            LOCK(cs_main);
            // slimcoin: try to select next block in a constant time
